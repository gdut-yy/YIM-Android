...
public class EmojiManager {
    ...
    static {
        Context context = EmotionKit.getContext();

        load(context, EMOJI_DIR + "emoji.xml");

        mPattern = makePattern();

        mDrawableCache = new LruCache<String, Bitmap>(CACHE_SIZE) {
            @Override
    ...
    private static final void load(Context context, String xmlPath) {
        ...
        //补充最后一页少的表情
        int tmp = mDefaultEntries.size() % EmoticonLayout.EMOJI_PER_PAGE;
        if (tmp != 0) {
            int tmp2 = EmoticonLayout.EMOJI_PER_PAGE - (mDefaultEntries.size() - (mDefaultEntries.size() / EmoticonLayout.EMOJI_PER_PAGE) * EmoticonLayout.EMOJI_PER_PAGE);
